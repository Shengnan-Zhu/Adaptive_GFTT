...
void MY_GFTT::goodFeaturesToTrack(cv::InputArray _image, cv::OutputArray _corners, int maxCorners, 
    ...
    if(corners.size() < 0.95 * maxCorners){
        re_extractor(image, mask, minDistance, int(maxCorners - corners.size()), rejectBorder, corners, scores, eig, tmp);
    }
...
void MY_GFTT::re_extractor(cv::InputArray _image, cv::InputArray &_mask, double minDistance, int maxCorners,
                            int &rejectBorder, vector<cv::Point2f> &_corners, std::vector<float> &_scores,
                            cv::InputArray _eigenMat, cv::InputArray _dilateMat){

    CV_Assert(_mask.empty() || (_mask.type() == CV_8UC1 && _mask.sameSize(_image)));
    cv::Mat image = _image.getMat(), eig = _eigenMat.getMat(), tmp = _dilateMat.getMat();
    if (image.empty() || eig.empty() || tmp.empty())
        return;
    ...
    for(size_t i = 0; i < _corners.size(); i++){
        double k = sqrt(_scores[i] / _scores[0]);
        double radius = minDistance * (1 - 0.5 * k);
        cv::circle(mask, _corners[i], std::max(radius, 10.0), 0, -1);
    }
    ...
    if (minDistance >= 1)
    {
        ...
        for( i = 0; i < total; i++ )
        {
            ...
            if (good)
            {
                ...
                _corners.push_back(cv::Point2f((float)x, (float)y));
                _scores.push_back(*tmpCorners[i]);
                ++ncorners;
